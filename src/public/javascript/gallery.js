(() => {
	...
	const generatePictureElement = (picture) => {
        ...
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 transition-all" fill="none" viewBox="0 0 24 24" stroke="currentColor" id="like-${picture.id}" stroke-width="2">
		...
		const applyLikeButtonStyles = () => {
			const likeId = picture.loggedUserLikeId;
			const likeSvg = likeButton.firstElementChild;

			if (typeof likeId === 'number') {
				likeButton.setAttribute('class', 'text-red-500');
		...
		const renderLikeCount = () => {
			likeCount.innerText = `${picture.likeCount} like${picture.likeCount > 1 ? 's' : ''}`;
		};

		likeButton.addEventListener('click', async (event) => {
			if (typeof picture.loggedUserLikeId === 'number') {
				const res = await fetch(`/api/pictures/${picture.id}/likes/${picture.loggedUserLikeId}`, {
					method: 'DELETE',
				});

				if (res.ok) {
					picture.loggedUserLikeId = null;
					--picture.likeCount;
				}
			} else {
				const res = await fetch(`/api/pictures/${picture.id}/likes`, {
					method: 'POST',
				});
				...
				if (res.ok) {
					...
					picture.loggedUserLikeId = likeId;
					++picture.likeCount;
				}
			}

			applyLikeButtonStyles();
			renderLikeCount();
		});
